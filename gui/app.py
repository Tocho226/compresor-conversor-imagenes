...
class ImageCompressorApp:
    ...
    def create_header(self, parent):
        ...
        style_title1.pop("bg", None)  # elimina 'bg' si existe
        style_title1["font"] = ("Arial", 25, "bold")
        ...
        self.title_label = tk.Label(header_frame, text="🎨 Compresor / Conversor Avanzado de Imágenes v9.1", **style_title1,
                                    bg=header_frame.cget("bg")) # << Esto hace que el fondo “transparente” se vea como el del contenedor
...
#        style_title2.pop("bg", None)  # elimina 'bg' si existe
...
#        style_title3.pop("bg", None)  # elimina 'bg' si existe
    ...
    #    # style.pop("bg", None)  # elimina 'bg' si existe
    #     style["font"] = ("Arial", 16, "bold")
    #    # style["fg"] = self.theme.get_color("text_primary")

    #     self.drop_label = tk.Label(
    #         self.drop_area,
    #         text="🖼️ Haz clic aquí para seleccionar Uno ó varios Archivos ",
    #         **style
    #    )

        style = self.theme.get_label_style("secondary").copy()
        # Asegurarnos de que el estilo no imponga un bg distinto al del contenedor
        style.pop("bg", None)
        # Forzar fg con el color de texto actual del tema
        style["fg"] = self.theme.get_color("text_primary")
        style["font"] = ("Arial", 16, "bold")

        # Crear el label usando el bg del contenedor (drop_area) para evitar recuadros
        self.drop_label = tk.Label(
            self.drop_area,
            text="🖼️ Haz clic aquí para seleccionar Uno ó varios Archivos ",
            bg=self.drop_area.cget("bg"),
            **style
        )
    ...
    def apply_theme_to_all_widgets(self):
        ...
        for widget_info in self.widgets_to_update:
            ...
            try:
                ...
                #     style = self.theme.get_label_style(style_type).copy()
                #     # Solo sobreescribir color, no la fuente
                #     font_actual = widget.cget("font")  # Obtener la fuente actual
                #     style["font"] = font_actual        # Mantener la fuente
                #     widget.configure(**style)

                elif widget_type == "label":
                    style = self.theme.get_label_style(style_type).copy()
                    # Mantener la fuente actual del widget
                    font_actual = widget.cget("font")
                    style["font"] = font_actual

                    # Si es el drop_label, quitar bg del estilo y forzar bg igual al drop_area
                    if hasattr(self, "drop_label") and widget == self.drop_label:
                        style.pop("bg", None)  # eliminar bg si el estilo lo trae
                        # Asegurar fg según tema
                        style["fg"] = self.theme.get_color("text_primary")
                        widget.configure(**style)
                        # Forzar fondo igual al del contenedor para evitar 'caja' al cambiar tema
                        widget.configure(bg=self.drop_area.cget("bg"))
                    else:
                        # Comportamiento normal para los demás labels
                        widget.configure(**style)
        ...
        if hasattr(self, 'title_label'):
            style_title = self.theme.get_label_style("title").copy()
            style_title["font"] = self.title_label.cget("font")  # Mantener la fuente

            # Actualizar color de texto
            self.title_label.configure(fg=style_title["fg"])
            # Actualizar fondo para que coincida con el contenedor (evita sombras)
            self.title_label.configure(bg=self.title_label.master.cget("bg"))
