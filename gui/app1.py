...
class ImageCompressorApp:
    ...
    #     # # Usar el mismo color de fondo que el contenedor para que se integre con el banner
    #     # # No se puede usar "" (vacío) porque Tkinter no lo acepta
    #     # self.title_label = tk.Label(banner_container, text="🎨 Compresor Avanzado de Imágenes v9.2", **style_title1)
    ...
    #     banner_container = tk.Frame(header_frame, bg=self.theme.get_color("bg_primary"))
    #     banner_container.grid(row=0, column=0, sticky="ew")
    #     banner_container.grid_columnconfigure(0, weight=1)
    #     banner_container.grid_rowconfigure(0, weight=1)
    #     self.widgets_to_update.append(("frame", banner_container, "primary"))

    #     self.banner_canvas.configure(bg=self.theme.get_color("bg_primary"))

    #     # Banner como imagen (degradado)
    #     banner_gradient = self.icons.get_gradient("banner")

    #     if banner_gradient:
    #         width = banner_gradient.width()
    #         height = banner_gradient.height()

    #         # Canvas para dibujar banner + título sin fondo
    ...
    #         #     bd=0,
    #         #     bg=self.theme.get_color("bg_primary")  # se oculta al estar cubierto por la imagen
    #         # )
    #         # self.banner_canvas.grid(row=0, column=0, sticky="ew")

    #         # # Dibujar la imagen del banner
    #         # self.banner_canvas.create_image(0, 0, anchor="nw", image=banner_gradient)

    #         # # Guardar ID del texto para actualizaciones posteriores
    #         # # # self.banner_title_id = self.banner_canvas.create_text(
    #         # # #     width // 2,
    #         # # #     height // 2,
    #         # # #     text="🎨 Compresor Avanzado de Imágenes v9.2",
    #         # # #     font=("Arial", 24, "bold"),
    #         # # #     fill=self.theme.get_color("text_on_primary")  # cambia según tema
    #         # # )
    ...
    #         #     anchor="center"
    #         # )
    #         # # Mantener título siempre centrado
    #         # def center_banner_title(event):
    #         #     self.banner_canvas.coords(self.banner_title_id, event.width // 2, event.height // 2)

    #         # self.banner_canvas.bind("<Configure>", center_banner_title)

    #         # Crear Canvas del banner
    #         self.banner_canvas = tk.Canvas(
    #             banner_container,
    #             width=banner_gradient.width(),
    #             height=banner_gradient.height(),
    ...
    #         # Dibujar imagen del banner
    #         self.banner_canvas.create_image(0, 0, anchor="nw", image=banner_gradient)
    #         self.banner_canvas.image = banner_gradient  # mantener referencia

    #         # Dibujar texto centrado y guardar ID
    #         self.banner_title_id = self.banner_canvas.create_text(
    #             0, 0,
    #             text="🎨 Compresor Avanzado de Imágenes v9.2",
    #             font=("Arial", 24, "bold"),
    ...
    #         # Ajustar posición del texto automáticamente
    #         def center_banner_title(event):
    #             self.banner_canvas.coords(self.banner_title_id, event.width // 2, event.height // 2)
    ...
    #         # Guardar referencia para el sistema de actualización del tema
    #         self.widgets_to_update.append(("banner_title", self.banner_canvas, None))
    ...
    def create_header(self, parent):
        ...
        # Crear canvas del banner siempre (evita errores)
        self.banner_canvas = tk.Canvas(
            banner_container,
            width=800,  # ancho por defecto si no hay imagen
            height=120,  # alto por defecto si no hay imagen
            highlightthickness=0,
            bd=0,
        ...
        if banner_gradient:
            ...
            # Ajustar tamaño del canvas al degradado
            self.banner_canvas.config(width=width, height=height)
        ...
        # Ajustar posición del texto automáticamente al redimensionar
        def center_banner_title(event):
            self.banner_canvas.coords(self.banner_title_id, event.width // 2, event.height // 2)
    ...
    def create_compression_settings(self, parent):
        ...
        # Configuración especial para Checkbutton para que funcione en ambos temas
        self.aspect_check = tk.Checkbutton(
            ...
            bg=self.theme.get_color("bg_primary"),
            fg=self.theme.get_color("text_primary"),
            activebackground=self.theme.get_color("bg_primary"),
            ...
            highlightcolor=self.theme.get_color("accent_primary"),
            relief="flat",
            borderwidth=0
        )
    ...
    #             # elif widget_type == "banner_title":
    #             #     # Actualizar color del texto cuando se cambie de tema
    #             #     widget.itemconfig(self.banner_title_id, fill=self.theme.get_color("text_on_primary"))
       
    #             # elif widget_type == "banner_title":
    #             #     widget.itemconfig(self.banner_title_id, fill=self.theme.get_color("text_on_primary"))
    #             #     # Actualizar fondo del canvas
    #             #     widget.configure(bg=self.theme.get_color("bg_primary"))
    #             elif widget_type == "banner_title":
    #                 # Cambiar color del texto
    #                 if hasattr(self, "banner_title_id") and self.banner_title_id is not None:
    #                     widget.itemconfig(self.banner_title_id, fill=self.theme.get_color("text_on_primary"))
    ...
    #                 # Checkbutton necesita configuración especial para funcionar en ambos temas
    #                 widget.configure(
    #                     bg=self.theme.get_color("bg_primary"),
    #                     fg=self.theme.get_color("text_primary"),
    #                     activebackground=self.theme.get_color("bg_primary"),
        ...
        for widget_info in self.widgets_to_update:
            ...
            try:
                if widget_type == "frame":
                    ...
                    # Actualizar color del texto y fondo solo si existen los objetos
                    if hasattr(self, "banner_canvas") and self.banner_canvas:
                        if hasattr(self, "banner_title_id") and self.banner_title_id:
                            widget.itemconfig(self.banner_title_id, fill=self.theme.get_color("text_on_primary"))
                        widget.configure(bg=self.theme.get_color("bg_primary"))
    ...
    def _compress_images_thread(self, output_dir: str, output_format: str, 
        ...
        try:
            ...
            self.root.after(0, lambda s=successful, f=failed, r=results: self._show_compression_results(s, f, r))
            
        except Exception as e:
            error_msg = str(e)  # Capturar el mensaje de error antes del lambda
            self.root.after(0, lambda msg=error_msg: messagebox.showerror("Error", f"Error durante la compresión: {msg}"))
